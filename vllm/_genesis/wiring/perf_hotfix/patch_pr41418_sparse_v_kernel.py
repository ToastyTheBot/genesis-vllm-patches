...
This is the second piece of PR41418 (TQ unified perf pack). The first piece
(centroids prebake) is in `patch_pr41418_tq_unified_perf.py`. This file wires
...
Composition
-----------
- **PR41418 main (centroids prebake)**: orthogonal, both can be enabled.
...
- **PR40941 TQ WorkspaceManager revert**: orthogonal — PR40941 patches
...
- **P67 multi-query kernel**: separate code path (spec-decode K+1
  ...
  PR41418 sparse_v ONLY applies to standard decode.
...
_GENESIS_PR41418_SPARSE_V_MARKER_ATTR = "_genesis_pn26_sparse_v_wrapped"
...
def _import_target() -> tuple[Any, Any] | None:
    ...
    try:
        ...
        log.warning("[Genesis PR41418 sparse_v] import %s failed: %s", _MODULE_PATH, e)
        return None
...
def apply() -> tuple[str, str]:
    ...
    # Idempotency
    if getattr(original, _GENESIS_PR41418_SPARSE_V_MARKER_ATTR, False):
    ...
    # ====================================================================
    # LEAN DISPATCHER (PR41422 v2, 2026-05-01) — no per-call GPU↔CPU sync.
    # ====================================================================
    ...
    if _baked_scale > 0:
        log.info(
            "[Genesis PR41418 sparse_v] BLASST adaptive (scale=%s) requested but "
            "per-call resolve costs more than skip savings on SM86. Using "
            "fixed threshold %s (override via GENESIS_PR41418_SPARSE_V_THRESHOLD).",
    ...
    if _baked_debug_skip:
        log.info(
            "[Genesis PR41418 sparse_v] DEBUG_SKIP_CTR=1 — per-CTA atomic "
    ...
    _call_counter = [0]
    _LOG_EVERY_N = int(os.environ.get("GENESIS_PR41418_SPARSE_V_LOG_EVERY", "500"))
    ...
    def _genesis_pn26_dispatcher(*args, **kwargs):
        ...
        try:
            ...
            log.warning(
                "[Genesis PR41418 sparse_v] kernel call failed (%s); "
            ...
            if _call_counter[0] % _LOG_EVERY_N == 0:
                try:
                    ...
                    if stats.get("enabled"):
                        ...
                        log.warning(
                            "[Genesis PR41418 sparse_v] skip-rate after %d calls: "
    ...
    setattr(_genesis_pn26_dispatcher, _GENESIS_PR41418_SPARSE_V_MARKER_ATTR, True)
    ...
    for consumer_path in _CONSUMER_MODULES:
        try:
            ...
            log.warning(
                "[Genesis PR41418 sparse_v] could not rebind on %s: %s",
                consumer_path, e,
            )

    log.info(
        "[Genesis PR41418 sparse_v] wrapped %s.%s with sparse-V dispatcher "
        "(also rebound on consumers: %s)",
        _MODULE_PATH, _FN_NAME, _rebound or "none",
    )
    return "applied", (
        "PR41418 sparse-V kernel dispatcher installed. Routes to Genesis "
        "fork when seq_len >= min_ctx (default 8192). Threshold: "
        "BLASST λ=a/L if GENESIS_PR41418_SPARSE_V_SCALE_FACTOR>0, else "
        "fixed GENESIS_PR41418_SPARSE_V_THRESHOLD (default 0.001). "
...
def is_applied() -> bool:
    ...
    if target is None:
        return False
    _, fn = target
    return bool(getattr(fn, _GENESIS_PR41418_SPARSE_V_MARKER_ATTR, False))
