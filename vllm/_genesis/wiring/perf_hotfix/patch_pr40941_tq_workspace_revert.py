...
GENESIS_PR40941_MARKER = (
    "Genesis PR40941 TQ WorkspaceManager revert (vllm#40941 perf hotfix) v7.62.14"
)
...
PR40941_DECODE_NEW = (
    "        # [Genesis PR40941 vllm#40941 perf revert] Skip WorkspaceManager.\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR40941 turboquant_attn.py — revert WorkspaceManager (perf hotfix)"
        ),
        target_file=str(target),
        marker=GENESIS_PR40941_MARKER,
        sub_patches=[
            TextPatch(
                name="p98_decode_workspace_revert",
                anchor=PR40941_DECODE_OLD,
                replacement=PR40941_DECODE_NEW,
                required=True,
            ),
            TextPatch(
                name="p98_dequant_workspace_revert",
                anchor=PR40941_DEQUANT_OLD,
                replacement=PR40941_DEQUANT_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40941 — TurboQuant WorkspaceManager revert."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR40941")
    log_decision("PR40941", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR40941] marker present — skip (idempotent)")
    ...
    if "current_workspace_manager" not in content:
        return "skipped", (
            ...
            "already reverted WorkspaceManager indirection. PR40941 unnecessary."
        )
    ...
    return (
        "applied",
        "PR40941 v7.62.14 applied: turboquant_attn.py _decode_attention + "
