...
- v794 27B Lorbus: currently `--gpu-memory-utilization=0.90` leaves
  ~2.0 GiB headroom per GPU. If PR41268 frees an extra 200-500 MiB,
...
- Restoration: on exit (success OR exception) the prior
  ...
  defaults to SIZE_MAX (no limit) — same as if PR41268 never ran.
...
To validate PR41268 actually helps on YOUR hardware before enabling
permanently:
...
4. If gap < 100 MiB, PR41268 is a no-op for your config. Disable.
...
another is anti-pattern. PR41268 default-OFF respects this. Do not flip
...
older torch versions the import is unsafe; PR41268 detects this and
...
GENESIS_PR41268_MARKER = "Genesis PR41268 scoped max_split_size_mb v1"
...
# context manager already wraps load_model) → PR41268 SKIPs cleanly →
# operator can retire the env flag.
UPSTREAM_DRIFT_MARKERS = [
    GENESIS_PR41268_MARKER,
...
PR41268_HELPER_NEW = (
    "    # [Genesis PR41268 scoped max_split_size_mb v1]\n"
    ...
    "                    'PR41268 scoped max_split_size_mb skipped (torch lacks '\n"
...
LOAD_MODEL_NEW = (
    ...
    "            self._maybe_get_memory_pool_context(tag=\"weights\"),\n"
    "            set_current_vllm_config(self.vllm_config),\n"
    "            # [Genesis PR41268] 20 MiB is PyTorch's minimum max_split_size_mb.\n"
...
def _patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41268 scoped max_split_size_mb",
        target_file=target,
        marker=GENESIS_PR41268_MARKER,
        sub_patches=[
            TextPatch(
                name="pn19_helper_method",
                anchor=PR41268_HELPER_OLD,
                replacement=PR41268_HELPER_NEW,
                required=True,
            ),
            TextPatch(
...
def apply() -> tuple[str, str]:
    """Apply PR41268. Default OFF. Opt-in via env flag.
    ...
    return result_to_wiring_status(result, failure, applied_message='PR41268 applied: max_split_size_mb=20 scoped to model load (vllm#41268 backport, MatthewBonanni). Frees 200-500 MiB load-time fragmentation.', patch_name='PR41268 scoped max_split_size_mb')
...
def is_applied() -> bool:
    ...
    try:
        with open(p.target_file) as f:
            return GENESIS_PR41268_MARKER in f.read()
    except Exception:
        return False
