...
GENESIS_PR41127_MARKER = (
    "Genesis PR41127 FlashInfer FULL CUDA graph for spec-decode (vllm#41127) v7.62.17"
)
...
PR41127_FISPECDECODE_NEW = (
    ...
    "\n"
    "\n"
    "# [Genesis PR41127 vllm#41127 backport] FISpecDecode dataclass for native\n"
...
PR41127_METADATA_DECODE_OLD = (
    "    decode: FIDecode | TRTLLMDecode | None\n"
)

PR41127_METADATA_DECODE_NEW = (
    "    # [Genesis PR41127 vllm#41127 backport] add FISpecDecode variant\n"
...
PR41127_CGSUPPORT_NEW = (
    '        """Get the cudagraph support level for FlashInfer attention.\n'
    "\n"
    "        [Genesis PR41127 vllm#41127 backport]\n"
...
PR41127_INIT_CGDICT_OLD = (
    "            self._decode_wrappers_cudagraph: dict[\n"
    "                int, BatchDecodeWithPagedKVCacheWrapper\n"
    "            ] = {}\n"
)

PR41127_INIT_CGDICT_NEW = (
    "            self._decode_wrappers_cudagraph: dict[\n"
    "                int, BatchDecodeWithPagedKVCacheWrapper\n"
    "            ] = {}\n"
    "            # [Genesis PR41127 vllm#41127 backport] Parallel dict for the\n"
...
PR41127_INIT_DECODE_WRAP_OLD = (
    "        self._decode_wrapper = None  # Wrapper for decode (general shape)\n"
)

PR41127_INIT_DECODE_WRAP_NEW = (
    "        self._decode_wrapper = None  # Wrapper for decode (general shape)\n"
    "        # [Genesis PR41127 vllm#41127 backport] Separate prefill-shaped\n"
...
PR41127_INIT_REORDER_OLD = (
    "        self._init_reorder_batch_threshold(1, supports_spec_as_decode=can_use_trtllm)\n"
)

PR41127_INIT_REORDER_NEW = (
    "        # [Genesis PR41127 vllm#41127 backport] Non-DCP native FlashInfer\n"
...
PR41127_INIT_BUFFER_OLD = (
    "        self.paged_kv_indices = self._make_buffer(max_num_pages)\n"
    "        self.paged_kv_last_page_len = self._make_buffer(max_num_reqs)\n"
)

PR41127_INIT_BUFFER_NEW = (
    "        self.paged_kv_indices = self._make_buffer(max_num_pages)\n"
    "        self.paged_kv_last_page_len = self._make_buffer(max_num_reqs)\n"
    "        # [Genesis PR41127 vllm#41127 backport] Persistent qo_indptr buffer\n"
...
PR41127_NEW_METHOD_OLD = (
    "        return decode_wrapper\n"
    "\n"
    "    def _get_cascade_wrapper(self):\n"
)

PR41127_NEW_METHOD_NEW = (
    "        return decode_wrapper\n"
    "\n"
    "    # ════════════════════════════════════════════════════════════════\n"
    "    # [Genesis PR41127 vllm#41127 backport] _get_spec_decode_prefill_wrapper\n"
    "    # ════════════════════════════════════════════════════════════════\n"
...
PR41127_BUILD_NEW = (
    "                # ════════════════════════════════════════════════════════════════\n"
    "                # [Genesis PR41127 vllm#41127 backport] Per-row qo_indptr delta scan\n"
    "                # ════════════════════════════════════════════════════════════════\n"
    ...
    "                    attn_metadata.decode = FIDecode(wrapper=decode_wrapper)\n"
    "                else:\n"
    "                    # [Genesis PR41127] Spec-decode: uniform query_len > 1\n"
...
PR41127_FORWARD_NEW = (
    "            if not decode_use_trtllm:\n"
    "                # [Genesis PR41127 vllm#41127 backport] Allow FISpecDecode too\n"
    ...
    "                    # [Genesis PR41127] Spec-decode verification through\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41127 flashinfer.py — native FULL CG for spec-decode (vllm#41127)",
        target_file=str(target),
        marker=GENESIS_PR41127_MARKER,
        sub_patches=[
            TextPatch(
                name="p100_imports_drop_uniform",
                anchor=PR41127_IMPORTS_OLD,
                replacement=PR41127_IMPORTS_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_fispecdecode_dataclass",
                anchor=PR41127_FISPECDECODE_OLD,
                replacement=PR41127_FISPECDECODE_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_metadata_decode_union",
                anchor=PR41127_METADATA_DECODE_OLD,
                replacement=PR41127_METADATA_DECODE_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_cgsupport_uniform_batch",
                anchor=PR41127_CGSUPPORT_OLD,
                replacement=PR41127_CGSUPPORT_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_init_decode_wrap_field",
                anchor=PR41127_INIT_DECODE_WRAP_OLD,
                replacement=PR41127_INIT_DECODE_WRAP_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_init_cgdict",
                anchor=PR41127_INIT_CGDICT_OLD,
                replacement=PR41127_INIT_CGDICT_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_init_reorder_threshold",
                anchor=PR41127_INIT_REORDER_OLD,
                replacement=PR41127_INIT_REORDER_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_init_qo_indptr_buffer",
                anchor=PR41127_INIT_BUFFER_OLD,
                replacement=PR41127_INIT_BUFFER_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_get_spec_decode_prefill_wrapper_method",
                anchor=PR41127_NEW_METHOD_OLD,
                replacement=PR41127_NEW_METHOD_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_build_query_len_scan_branch",
                anchor=PR41127_BUILD_OLD,
                replacement=PR41127_BUILD_NEW,
                required=True,
            ),
            TextPatch(
                name="p100_forward_fispecdecode_case",
                anchor=PR41127_FORWARD_OLD,
                replacement=PR41127_FORWARD_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR41127 v1 — FlashInfer FULL CG for spec-decode (vllm#41127 backport).
    ...
    Composability:
    - PROD (TurboQuantAttentionImpl backend) — NOT affected, PR41127 only
      patches FlashInferImpl. Co-exists with P67/P67b/PR40941/PR40941b.
    ...
    decision, reason = should_apply("PR41127")
    log_decision("PR41127", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR41127] marker present — skip (idempotent)")
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR41127 v7.62.17 applied: 11 sub-patches on flashinfer.py for native "
            ...
            "Composes with P67/P67b/PR40941/PR40941b (different backends)."
        ),
