...
GENESIS_PR39148_MARKER = "Genesis PR39148 merge_attn_states NaN guard (vllm#39148) v7.65"
...
PR39148_MAX_LSE_NEW = (
    "    max_lse = tl.maximum(p_lse, s_lse)\n"
    "\n"
    "    # [Genesis PR39148] vllm#39148 backport — NaN guard for both-LSE-(-inf) edge case.\n"
...
PR39148_OUT_SE_NEW = (
    "    p_se = tl.exp(p_lse)\n"
    "    s_se = tl.exp(s_lse)\n"
    "    # [Genesis PR39148] vllm#39148 backport — epsilon prevents 0/0 in division below.\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR39148 triton_merge_attn_states.py — NaN guard for both-LSE-(-inf) "
            "edge case (vllm#39148 backport)"
        ),
        target_file=str(target),
        marker=GENESIS_PR39148_MARKER,
        sub_patches=[
            TextPatch(
                name="pn28_max_lse_clamp",
                anchor=PR39148_MAX_LSE_OLD,
                replacement=PR39148_MAX_LSE_NEW,
                required=True,
            ),
            TextPatch(
                name="pn28_out_se_epsilon",
                anchor=PR39148_OUT_SE_OLD,
                replacement=PR39148_OUT_SE_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR39148 — merge_attn_states NaN guard."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR39148")
    log_decision("PR39148", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR39148 applied: merge_attn_states Triton kernel now branchless-"
...
def is_applied() -> bool:
    ...
    try:
        with open(str(target)) as f:
            return GENESIS_PR39148_MARKER in f.read()
    except OSError:
        return False
