...
- **2026-05-03 v7.69.x** — Genesis port of PR #35975, prompted by
  ...
  (`patch_inputs_embeds_optional.py`) on club-3090 dev tip; this PR35975
...
GENESIS_PR35975_MARKER = (
    "Genesis PR35975 inputs_embeds optional for text-only (vllm#35975) v7.69"
)
...
PR35975_PART1_REPLACEMENT = (
    "        # [Genesis PR35975 v7.69 inputs_embeds_optional] vllm#35975 by\n"
...
def _make_patcher_part1() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR35975 v1/worker/gpu_model_runner.py — skip inputs_embeds "
            "buffer for text-only (vllm#35975)"
        ),
        target_file=str(target),
        marker=GENESIS_PR35975_MARKER + " part1",
        sub_patches=[
            TextPatch(
                name="pN35_gpu_model_runner_inputs_embeds_optional",
                anchor=PR35975_PART1_ANCHOR,
                replacement=PR35975_PART1_REPLACEMENT,
                required=True,
            ),
        ],
        upstream_drift_markers=[
            ...
            "[Genesis PR35975 v7.69 inputs_embeds_optional]",
...
def _make_patcher_part2() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR35975 v1/spec_decode/llm_base_proposer.py — skip "
            "inputs_embeds tensor for text-only proposers (vllm#35975)"
        ),
        target_file=str(target),
        marker=GENESIS_PR35975_MARKER + " part2",
        sub_patches=[
            TextPatch(
                name="pN35_llm_base_proposer_inputs_embeds_optional",
                anchor=PR35975_PART2_ANCHOR,
                replacement=PR35975_PART2_REPLACEMENT,
                required=True,
            ),
        ],
        upstream_drift_markers=[
...
def apply() -> tuple[str, str]:
    """Apply PR35975 — skip inputs_embeds buffer on text-only models.

    Two-file text-patch (gpu_model_runner + llm_base_proposer).
    Default ON (when env-flag GENESIS_DISABLE_PR35975_INPUTS_EMBEDS_OPTIONAL
    ...
    decision, reason = should_apply("PR35975")
    log_decision("PR35975", decision, reason)
    if not decision:
    ...
    for label, result, failure in patch_results:
        if result not in (
            ...
            log.warning(
                "[Genesis PR35975] %s did not apply: %s — %s "
    ...
    if not (any_applied or any_idempotent):
        return "failed", (
            "PR35975: no sub-patch applied — vllm pin may have rewritten "
    ...
    return result_to_wiring_status(
        ...
        None,
        applied_message=(
            "PR35975 v7.69 applied: inputs_embeds buffer now skipped on "
        ...
        patch_name="PR35975 inputs_embeds optional for text-only",
    )
