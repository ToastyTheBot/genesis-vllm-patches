...
shapes `d ∈ {64, 128, 256} × bits ∈ {3, 4, 8}`. ALWAYS-ON when PR41418 is
...
- `GENESIS_PR41418_SPARSE_V_THRESHOLD` — softmax probability cutoff
  (float, default 0.001). Below this max prob, the V tile is skipped.

- `GENESIS_PR41418_SPARSE_V_CTX_THRESHOLD` — minimum context length
...
- Composes with our PR40941 (TQ WorkspaceManager revert) without conflict —
  PR41418 patches `centroids.py` (separate file from `_decode_attention`)
  and `triton_turboquant_decode.py` kernel definition (separate from
  PR40941's invocation-site patch).
- Composes with P67 (multi-query Triton kernel) — P67 replaces the
  K+1 verify path; PR41418 leaves that path untouched and only modifies
  the standard decode path.
- Composes with PR40849 (MTP draft online quant) — orthogonal.
...
GENESIS_PR41418_MARKER = "Genesis PR41418 TQ unified perf (centroids+sparseV) v7.65"


# ─── Sub-patch 1: centroids.py — pre-baked tables + self-check ────────
PR41418_CENTROIDS_ANCHOR = (
...
PR41418_CENTROIDS_REPLACEMENT = (
    "# [Genesis PR41418] Pre-baked Lloyd-Max centroids — backport of vllm#41418\n"
    ...
    "_GENESIS_PR41418_PREBAKED: dict[tuple[int, int], tuple[float, ...]] = {\n"
    ...
    "_GENESIS_PR41418_LAZY_LOADED: bool = False\n"
    "_GENESIS_PR41418_DRIFT_DETECTED: bool = False\n"
    "\n"
    "\n"
    ...
    "    \"\"\"\n"
    "    global _GENESIS_PR41418_LAZY_LOADED, _GENESIS_PR41418_DRIFT_DETECTED\n"
    "    if _GENESIS_PR41418_LAZY_LOADED:\n"
    "        return\n"
    "    _GENESIS_PR41418_LAZY_LOADED = True\n"
    ...
    "            _GENESIS_PR41418_PREBAKED[(128, 4)], dtype=torch.float32\n"
    "        )\n"
    ...
    "        if max_diff > 1e-3:\n"
    "            _GENESIS_PR41418_DRIFT_DETECTED = True\n"
    "            import logging as _logging\n"
    "            _logging.getLogger(\"genesis.pn26\").warning(\n"
    "                \"[Genesis PR41418] Centroid prebake self-check FAILED: \"\n"
    ...
    "                \"[Genesis PR41418] Centroid prebake self-check OK with minor \"\n"
    ...
    "    \"\"\"Get precomputed Lloyd-Max centroids (cached).\n"
    "\n"
    "    Genesis PR41418: short-circuits to a pre-baked table for known\n"
    ...
    "    if not _GENESIS_PR41418_DRIFT_DETECTED:\n"
    "        prebaked = _GENESIS_PR41418_PREBAKED.get((d, bits))\n"
...
def _make_centroids_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR41418 centroids.py — pre-baked Lloyd-Max tables + self-check "
            "(backport of vllm#41418 with Genesis drift defense)"
        ),
        target_file=str(target),
        marker=GENESIS_PR41418_MARKER + " (centroids)",
        sub_patches=[
            TextPatch(
                name="pn26_centroids_prebake",
                anchor=PR41418_CENTROIDS_ANCHOR,
                replacement=PR41418_CENTROIDS_REPLACEMENT,
                required=True,
            ),
        ],
        upstream_drift_markers=[
            "[Genesis PR41418]",
            "_GENESIS_PR41418_PREBAKED",
...
def _apply_centroids() -> tuple[str, str]:
    ...
    if result == TextPatchResult.APPLIED:
        return "applied", "PR41418 centroids prebaked + self-check active"
...
def apply() -> tuple[str, str]:
    """Apply PR41418 — TQ unified perf pack (centroids prebake + sparse V scaffold).
    ...
    decision, reason = should_apply("PR41418")
    log_decision("PR41418", decision, reason)
    if not decision:
    ...
    log.info("[PR41418:centroids] %s — %s", cent_status, cent_detail)
    ...
    if cent_status == "applied":
        return "applied", (
            f"PR41418 centroids prebaked active (with drift self-check); "
            f"sparse V {sparse_v_status}"
        )
    if cent_status == "skipped":
        return "skipped", f"PR41418 centroids: {cent_detail}; sparse V {sparse_v_status}"
    return "failed", f"PR41418 centroids failed: {cent_detail}"
