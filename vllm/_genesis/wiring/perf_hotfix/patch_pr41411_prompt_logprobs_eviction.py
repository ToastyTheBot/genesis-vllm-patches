...
"""Wiring for PR41411 — vllm#41411 backport: prompt_logprobs eviction fix.
...
---------------------
PR41411 mutates 3 files. Each sub-patch is `required=True`. As of v7.70 the
...
GENESIS_PR41411_MARKER = "Genesis PR41411 prompt_logprobs eviction fix v7.66 (vllm#41411 backport)"
...
PROMPT_LOGPROB_NEW = (
    "        # [Genesis PR41411 vllm#41411] Drop overly-aggressive -1 — caused\n"
...
INPUT_BATCH_FIELD_NEW = (
    "    lora_request: LoRARequest | None = None\n"
    "    prompt_embeds: torch.Tensor | None = None\n"
    "    # [Genesis PR41411 vllm#41411] Per-request accumulator for prompt logprobs\n"
...
INPUT_BATCH_INIT_NEW = (
    "        # [Genesis PR41411 vllm#41411] in_progress_prompt_logprobs_cpu moved\n"
...
INPUT_BATCH_REMOVE_NEW = (
    "        self.logprob_token_ids.pop(req_id, None)\n"
    "        # [Genesis PR41411 vllm#41411] dict pop removed — accumulator now\n"
...
RUNNER_DICT_ALIAS_NEW = (
    ...
    "            return {}\n"
    "\n"
    "        # [Genesis PR41411 vllm#41411] in_progress_dict alias removed —\n"
...
RUNNER_GET_NEW = (
    "            # Set up target LogprobsTensors object.\n"
    "            # [Genesis PR41411 vllm#41411] read accumulator from request,\n"
...
RUNNER_DEL_NEW = (
    "        for req_id in completed_prefill_reqs:\n"
    "            del num_prompt_logprobs_dict[req_id]\n"
    "            # [Genesis PR41411 vllm#41411] clear accumulator on request, not dict\n"
...
def _make_patcher_prompt_logprob() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41411 prompt_logprob.py (drop -1)",
        target_file=str(target),
        marker=GENESIS_PR41411_MARKER,
...
def _make_patcher_input_batch() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41411 gpu_input_batch.py (move accumulator)",
        target_file=str(target),
        marker=GENESIS_PR41411_MARKER,
        sub_patches=[
...
def _make_patcher_model_runner() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41411 gpu_model_runner.py (per-request accumulator)",
        target_file=str(target),
        marker=GENESIS_PR41411_MARKER,
        sub_patches=[
...
def apply() -> tuple[str, str]:
    ...
    log_decision("PR41411", decision, reason)
    if not decision:
    ...
    txn = MultiFilePatchTransaction(patchers, name="PR41411")
