...
GENESIS_PR40849_MARKER = (
    "Genesis PR40849 MTP/draft online-quant propagation (vllm#40849) v7.62.x"
)
...
PR40849_IMPORTS_NEW = (
    ...
    "    QuantizationConfig,\n"
    ")\n"
    "# [Genesis PR40849 vllm#40849 backport] OnlineQuantizationConfig import\n"
    ...
    "        OnlineQuantizationConfig as _GENESIS_PR40849_OnlineQuantizationConfig,\n"
    "    )\n"
    "except Exception:  # pragma: no cover - module shape variance across vllm builds\n"
    "    _GENESIS_PR40849_OnlineQuantizationConfig = None\n"
)
...
PR40849_BODY_NEW = (
    "    # ════════════════════════════════════════════════════════════════\n"
    "    # [Genesis PR40849 vllm#40849 backport] online-quant propagation\n"
    ...
    "            try:\n"
    "                logger.warning(\n"
    "                    \"[Genesis PR40849] Draft model has quantization=%s but \"\n"
    ...
    "    if (\n"
    "        _GENESIS_PR40849_OnlineQuantizationConfig is not None\n"
    "        and isinstance(\n"
    "            getattr(vllm_config, \"quant_config\", None),\n"
    "            _GENESIS_PR40849_OnlineQuantizationConfig,\n"
    "        )\n"
    "    ):\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR40849 model_executor/models/utils.py — get_draft_quant_config "
            "online-quant propagation (vllm#40849)"
        ),
        target_file=str(target),
        marker=GENESIS_PR40849_MARKER,
        sub_patches=[
            TextPatch(
                name="pN8_imports",
                anchor=PR40849_IMPORTS_OLD,
                replacement=PR40849_IMPORTS_NEW,
                required=True,
            ),
            TextPatch(
                name="pN8_body",
                anchor=PR40849_BODY_OLD,
                replacement=PR40849_BODY_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40849 — MTP/draft online-quant propagation (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR40849")
    log_decision("PR40849", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR40849] marker present — skip (idempotent)")
    ...
    if (
        ...
        and "[Genesis PR40849" not in content
    ):
        return (
            "skipped",
            "upstream drift: OnlineQuantizationConfig already imported in "
            "utils.py — PR #40849 (or equivalent) appears merged; PR40849 self-retires",
        )
    ...
    return (
        "applied",
        "PR40849 applied: get_draft_quant_config() now inherits target's "
