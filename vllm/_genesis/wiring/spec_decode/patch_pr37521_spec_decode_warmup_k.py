...
- **Default**: ON when `num_speculative_tokens > 1` (real correctness
  ...
  `GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K=1` if warmup itself OOMs
  on a tight rig.
- **Idempotent**: marker `Genesis PR37521 spec-decode warmup K-aware`
  pin-checked.
...
- ampersandru `propose_draft_token_ids` mid-stream OOM should disappear
  on club-3090 1× 3090 long-vision 140K + MTP K=3 with PR37521 enabled.
- noonghunna workspace-lock `AssertionError` on dev205 MTP K=3
  single-card should disappear with PR37521 enabled.
...
GENESIS_PR37521_MARKER = (
    "Genesis PR37521 spec-decode warmup K-aware "
...
#
# Env override: GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K=1 forces
# the original [0] behavior for tight-VRAM rigs where the K-sized
# warmup itself OOMs.
PR37521_REPLACEMENT = (
    "        if self.speculative_config:\n"
    "            # [Genesis PR37521 vllm#37521-extended]\n"
    ...
    "                'GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K', ''\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR37521 v1/worker/gpu_model_runner.py — spec-decode warmup "
            "K-aware sizing (vllm#37521 extended to MTP/ngram)"
        ),
        target_file=str(target),
        marker=GENESIS_PR37521_MARKER,
        sub_patches=[
            TextPatch(
                name="pN33_warmup_k_draft_tokens",
                anchor=PR37521_ANCHOR,
                replacement=PR37521_REPLACEMENT,
                required=True,
            ),
        ],
        upstream_drift_markers=[
            ...
            # would false-positive (PR37521 v1 had this bug, observed on
            # live boot 2026-05-02 — server skipped PR37521 incorrectly).
...
def apply() -> tuple[str, str]:
    """Apply PR37521 — spec-decode warmup K-aware sizing.

    Default ON when spec-decode is active. Env override
    GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K=1 reverts to [0].
    """
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR37521")
    log_decision("PR37521", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR37521] marker present — skip (idempotent)")
    ...
    for m in patcher.upstream_drift_markers:
        ...
        if m in content:
            return (
                ...
                "Re-evaluate whether Genesis PR37521 extension is still "
    ...
    return (
        "applied",
        "PR37521 applied: spec-decode warmup uses real num_speculative_tokens "
        ...
        "GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K=1 if warmup OOMs."
    )
