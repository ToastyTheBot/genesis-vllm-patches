...
latency, hence Genesis Issue should report PR40941b alongside mean).
...
GENESIS_PR41043_MARKER = (
    "Genesis PR41043 spec-decode prepare_next_token_ids_padded zero-alloc "
    "(vllm#41043) v7.62.7"
)
...
PR41043_NEW = (
    "        # ════════════════════════════════════════════════════════════════\n"
    "        # [Genesis PR41043 vllm#41043 backport] Zero-alloc precompute of\n"
    "        # backup token IDs. Eliminates GPU->CPU .tolist() sync, list-\n"
    "        # comprehension Python objects, and np.array(...) allocation\n"
    "        # in the hot path. PR author measured PR40941b TPOT -9.3% on\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR41043 v1/spec_decode/llm_base_proposer.py — zero-alloc "
            "prepare_next_token_ids_padded (vllm#41043)"
        ),
        target_file=str(target),
        marker=GENESIS_PR41043_MARKER,
        sub_patches=[
            TextPatch(
                name="p94_zero_alloc_backup_token_ids",
                anchor=PR41043_OLD,
                replacement=PR41043_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR41043 — spec-decode prepare_next_token_ids_padded zero-alloc."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR41043")
    log_decision("PR41043", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR41043] marker present — skip (idempotent)")
    ...
    return (
        "applied",
        "PR41043 applied: prepare_next_token_ids_padded uses in-place loop "
