...
GENESIS_ENABLE_PR40610_ASYNC_PROPOSER_SYNC=1
...
GENESIS_P79B_MARKER = "Genesis PR40610 async proposer-sync backport vllm#40610 v7.46"
...
P79B_NEW = (
    ...
    "    ) -> ModelRunnerOutput | AsyncModelRunnerOutput | IntermediateTensors:\n"
    "        # ════════════════════════════════════════════════════════════\n"
    "        # [Genesis PR40610 backport vllm#40610] Re-record prepare_inputs_event\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40610 v1/worker/gpu_model_runner.py — async proposer-sync backport",
...
def apply() -> tuple[str, str]:
    """Apply PR40610 — async-scheduling × spec-decode proposer sync fix."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR40610")
    log_decision("PR40610", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR40610] marker present — skip (idempotent)")
    ...
    if "_sample_tokens_impl" in content and "[Genesis PR40610" not in content:
        return "skipped", (
    ...
    return "applied", (
        "PR40610 applied: sample_tokens() now wraps _sample_tokens_impl with "
