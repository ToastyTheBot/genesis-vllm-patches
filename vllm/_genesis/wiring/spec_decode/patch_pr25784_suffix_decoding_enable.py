...
sees clear error and falls back to ngram by removing PR25784 env).
...
GENESIS_PR25784_MARKER = "Genesis PR25784 suffix-decoding auto-enable v7.56_local_os_import"
...
# is the start of the ngram-only configuration branch. We insert PR25784 swap
...
PR25784_NEW = (
    ...
    "\n"
    "        # ════════════════════════════════════════════════════════════\n"
    "        # [Genesis PR25784 vllm#25784] Auto-swap ngram -> suffix when env enabled.\n"
    ...
    "            except ImportError:\n"
    "                logger.warning(\n"
    "                    \"[Genesis PR25784] arctic_inference not installed -- \"\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR25784 config/speculative.py — auto-enable suffix decoding",
        target_file=str(target),
        marker=GENESIS_PR25784_MARKER,
        sub_patches=[
            TextPatch(
                name="p75_suffix_swap",
                anchor=PR25784_OLD,
                replacement=PR25784_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR25784 — suffix decoding auto-enable."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR25784")
    log_decision("PR25784", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR25784] marker present — skip (idempotent)")
        return "applied", "idempotent (marker present)"
    for m in patcher.upstream_drift_markers:
        if m == "[Genesis PR25784" and m in content:
            continue
        if m in content:
            return (
                "skipped",
