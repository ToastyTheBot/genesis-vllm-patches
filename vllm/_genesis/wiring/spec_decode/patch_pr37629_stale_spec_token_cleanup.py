...
- Composes cleanly with PR40610 (async proposer-sync) — orthogonal location
...
GENESIS_ENABLE_PR37629_STALE_SPEC_TOKEN_CLEANUP=1
...
GENESIS_P79C_MARKER = "Genesis PR37629 stale spec_token_ids cleanup backport vllm#37629 v7.46"
...
P79C_NEW = (
    "        # ════════════════════════════════════════════════════════════\n"
    "        # [Genesis PR37629 backport vllm#37629 — v7.49 improvement]\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR37629 v1/core/sched/scheduler.py — stale spec_token_ids cleanup",
...
def apply() -> tuple[str, str]:
    """Apply PR37629 — stale spec_token_ids cleanup."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR37629")
    log_decision("PR37629", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR37629] marker present — skip (idempotent)")
    ...
    if upstream_marker in content and "[Genesis PR37629" not in content:
        return "skipped", (
    ...
    return "applied", (
        "PR37629 applied: schedule() now clears spec_token_ids for unscheduled "
