...
"""Wiring for Patch 60b — GDN+ngram Triton kernel offset (PR40738 Phase 2).

Backport of vllm-project/vllm#40738 (Thomas Parnell), Triton kernel portion.

================================================================
DEPENDS ON PR40738 (Phase 1). Apply PR40738 BEFORE PR40738b. Without Phase 1,
the SSM state pre-copy isn't done so the conv state offset alone
won't help.

PR40738 Phase 1 result on dev205: 13/30 clean (43%) vs baseline 6/30 (~20%)
...
(currently they don't even though the kernel parameter is there from PR40738
Phase 1).
...
GENESIS_P60B_MARKER = "Genesis PR40738b GDN+ngram Triton kernel offset v7.13"
...
KERNEL_SIG_NEW = (
    "    initial_state_idx,  # (batch,)\n"
    "    num_computed_tokens,  # (batch,)\n"
    "    num_accepted_tokens_ptr,  # (batch,) or None  [Genesis PR40738b vllm#40738]\n"
    "    o_ptr,  # (dim, seqlen) - actually pointing to x_ptr"
)
...
KERNEL_CONSTEXPR_NEW = (
    "    IS_APC_ENABLED: tl.constexpr,\n"
    "    IS_SPEC_DECODING: tl.constexpr,  # [Genesis PR40738b vllm#40738]\n"
    "    HAS_NULL_BLOCK: tl.constexpr,"
)
...
KERNEL_OFFSET_NEW = (
    ...
    "    idx_seq = tl.load(batch_ptr + tl.program_id(0)).to(tl.int64)\n"
    "\n"
    "    # [Genesis PR40738b vllm#40738] Compute conv state token offset.\n"
    "    if IS_SPEC_DECODING:\n"
...
KERNEL_STEP1_NEW = (
    "        if load_init_state:\n"
    "            # load from conv_states  [Genesis PR40738b vllm#40738: + conv_state_token_offset]\n"
    "            prior_tokens = (\n"
    "                conv_states_base\n"
...
KERNEL_STEP2_NEW = (
    ...
    "                    + (conv_states_input_coord * stride_conv_state_seq)\n"
    "                    + (idx_feats * stride_conv_state_dim)[None, :]\n"
    "                    # [Genesis PR40738b vllm#40738: + conv_state_token_offset]\n"
    "                    + (\n"
...
WRAPPER_SIG_NEW = (
    ...
    "    initial_state_idx: torch.Tensor | None = None,\n"
    "    num_computed_tokens: torch.Tensor | None = None,\n"
    "    num_accepted_tokens: torch.Tensor | None = None,  # [Genesis PR40738b vllm#40738]\n"
    "    block_size_to_align=0,\n"
    "    metadata=None,\n"
    "    validate_data=False,\n"
    "):"
...
KERNEL_CALL_NEW = (
    "        initial_state_idx,\n"
    "        num_computed_tokens,\n"
    "        num_accepted_tokens,  # [Genesis PR40738b vllm#40738]\n"
    "        out,"
)
...
KERNEL_KWARG_NEW = (
    "        IS_APC_ENABLED=block_idx_last_scheduled_token is not None,\n"
    "        IS_SPEC_DECODING=num_accepted_tokens is not None,  # [Genesis PR40738b vllm#40738]\n"
    "        HAS_NULL_BLOCK=null_block_id is not None,"
)
...
def _make_kernel_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40738b causal_conv1d.py — Triton kernel + wrapper",
...
GDN_CONV_FN_NEW = (
    "            # [Genesis PR40738b vllm#40738] gate num_accepted_tokens on spec activity\n"
...
def _make_gdn_caller_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40738b gdn_linear_attn.py — pass num_accepted to conv_fn",
        target_file=str(target),
...
def _clear_triton_cache() -> tuple[bool, str]:
    ...
    for d in cache_dirs:
        ...
        try:
            ...
            log.debug("[Genesis PR40738b] cache clear failed for %s: %s", d, e)
    return False, "no cache dir cleared (may be empty already or permission denied)"


def apply() -> tuple[str, str]:
    """Apply PR40738b (Phase 2) Triton kernel patch + gdn caller passthrough."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR40738b")
    log_decision("PR40738b", decision, reason)
    if not decision:
    ...
    log.info("[Genesis PR40738b] Triton cache: %s", cache_msg)
    ...
    return "applied", (
        f"PR40738b Phase 2 applied: {applied} files modified, {idempotent} "
