...
disproof of PR40768 (#40768 backport) and PR39055 (#39055 backport).
================================================================
...
Empirical isolation 2026-04-25 (blue/green tests on Genesis pin):
    ...
    spec=ngram + tools + PR40768            4/5  broken
    spec=ngram + tools + PR39055            3/5  broken
    spec=ngram + tools + cudagraph=NONE 3/5  broken
    spec=ngram_gpu + PR40768 (Path B)       worse (5/5 broken)
...
This patch (PR40738 Phase 1) backports the PYTHON-ONLY portions:
...
GENESIS_PR40738_MARKER = "Genesis PR40738 GDN+ngram state recovery v7.13"
...
GDN_ATTN_FIELD_NEW = (
    "    num_accepted_tokens: torch.Tensor | None = None  # shape: [batch,]\n"
    "\n"
    "    # [Genesis PR40738 vllm#40738] 1D source block indices for state recovery\n"
...
GDN_ATTN_BUILD_NEW = (
    "        # [Genesis PR40738 vllm#40738] init recovery indices\n"
    ...
    "            non_spec_query_start_loc = query_start_loc\n"
    "            non_spec_query_start_loc_cpu = query_start_loc_cpu\n"
    "            # [Genesis PR40738 vllm#40738] gate num_accepted_tokens on spec activity\n"
    "            if (\n"
...
def _make_gdn_attn_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40738 gdn_attn.py — spec_decode_src_indices",
        target_file=str(target),
        marker=GENESIS_PR40738_MARKER + " :: gdn_attn.py",
        sub_patches=[
        ...
        # field comment; we use "[Genesis PR40738 ...]" prefix. This way our
        # PR40738b's reference to `spec_decode_src_indices` doesn't false-trigger.
...
GDN_LINATTN_CORE_NEW = (
    ...
    "        num_accepted_tokens = attn_metadata.num_accepted_tokens\n"
    "\n"
    "        # [Genesis PR40738 vllm#40738] SSM state pre-copy from accepted block\n"
...
def _make_gdn_linattn_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40738 gdn_linear_attn.py — SSM state pre-copy",
        target_file=str(target),
        marker=GENESIS_PR40738_MARKER + " :: gdn_linear_attn.py",
        sub_patches=[
...
GMR_NEW = (
    ...
    "                )\n"
    "            elif (\n"
    "                # [Genesis PR40738 vllm#40738] non-spec steps with spec_config: pass num_accepted\n"
    "                not use_spec_decode\n"
...
def _make_gmr_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40738 gpu_model_runner.py — non-spec passthrough",
        target_file=str(target),
        marker=GENESIS_PR40738_MARKER + " :: gpu_model_runner.py",
        sub_patches=[
            TextPatch(name="p60_passthrough", anchor=GMR_OLD,
                      replacement=GMR_NEW, required=True),
        ],
        upstream_drift_markers=["[Genesis PR40738 vllm#40738] non-spec steps"],
    )


def apply() -> tuple[str, str]:
    """Apply PR40738 Phase 1 (Python-only) backport. All-or-nothing."""
        ...
        for sp in p.sub_patches:
            if sp.required and sp.anchor not in content:
                return (
                    ...
                    f"{p.target_file} — anchor drifted, PR40738 cannot apply.",
                )
    ...
    return "applied", (
        f"PR40738 Phase 1 applied: {applied} files modified, {idempotent} "
