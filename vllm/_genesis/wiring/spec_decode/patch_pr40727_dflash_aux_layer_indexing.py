...
GENESIS_PR40727_MARKER = "Genesis PR40727 DFlash aux layer indexing +1 shift v7.65"
...
PR40727_REPLACEMENT = (
    ...
    "            dflash_config = getattr(hf_config, \"dflash_config\", None)\n"
    "            if dflash_config and isinstance(dflash_config, dict):\n"
    "                # [Genesis PR40727] vllm#40727 backport — DFlash target_layer_ids\n"
...
def apply() -> tuple[str, str]:
    """Apply PR40727 — DFlash aux layer +1 shift (vllm#40727)."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR40727")
    log_decision("PR40727", decision, reason)
    if not decision:
    ...
    patcher = TextPatcher(
        patch_name="PR40727 gpu_model_runner.py — DFlash aux layer +1 shift (vllm#40727)",
        target_file=str(target),
        marker=GENESIS_PR40727_MARKER,
        sub_patches=[
            TextPatch(
                name="pn24_aux_layer_shift",
                anchor=PR40727_ANCHOR,
                replacement=PR40727_REPLACEMENT,
                required=True,
            ),
        ],
    ...
    return result_to_wiring_status(
        ...
        failure,
        applied_message=(
            "PR40727 applied: DFlash aux_layer_ids now shifted +1 in "
            "_get_eagle3_aux_layers_from_config (vllm#40727 backport). "
            "AL gsm8k 6.18→6.42 per PR author measurement."
        ),
        patch_name="PR40727 DFlash aux layer indexing +1 shift",
    )
...
def is_applied() -> bool:
    ...
    try:
        with open(str(target)) as f:
            return GENESIS_PR40727_MARKER in f.read()
    except OSError:
        return False
