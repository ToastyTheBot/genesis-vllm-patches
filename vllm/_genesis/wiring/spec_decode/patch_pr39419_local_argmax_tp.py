...
- NO callsite swap: this patch only adds the methods. The proposer
  ...
  in our pin). So enabling PR39419 == enabling vocab-parallel argmax.
...
GENESIS_PR39419_MARKER = "Genesis PR39419 local argmax TP for spec-decode draft v7.65"

# ─── Sub-patch 1: qwen3.py ──────────────────────────────────────────
PR39419_QWEN3_ANCHOR = (
    "    def compute_logits(\n"
    "        self,\n"
...
PR39419_QWEN3_REPLACEMENT = (
    ...
    "        hidden_states: torch.Tensor,\n"
    "    ) -> torch.Tensor:\n"
    "        # [Genesis PR39419] vllm#39419 backport — vocab-parallel argmax,\n"
...
PR39419_DFLASH_REPLACEMENT = (
    ...
    "        hidden_states: torch.Tensor,\n"
    "    ) -> torch.Tensor:\n"
    "        # [Genesis PR39419] vllm#39419 backport — vocab-parallel argmax for DFlash\n"
...
def apply() -> tuple[str, str]:
    """Apply PR39419 — vocab-parallel argmax plumbing (vllm#39419)."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR39419")
    log_decision("PR39419", decision, reason)
    if not decision:
    ...
    patcher_qwen3 = TextPatcher(
        patch_name="PR39419 qwen3.py — get_top_tokens (vllm#39419)",
        target_file=str(qwen3),
        marker=GENESIS_PR39419_MARKER,
        sub_patches=[
            TextPatch(
                name="pn22_qwen3_get_top_tokens",
                anchor=PR39419_QWEN3_ANCHOR,
                replacement=PR39419_QWEN3_REPLACEMENT,
                required=True,
            ),
        ],
    ...
    if dflash is not None and os.path.isfile(str(dflash)):
        patcher_dflash = TextPatcher(
            patch_name="PR39419 qwen3_dflash.py — get_top_tokens (vllm#39419)",
            target_file=str(dflash),
            marker=GENESIS_PR39419_MARKER + " (dflash)",
            sub_patches=[
                TextPatch(
                    name="pn22_dflash_get_top_tokens",
                    anchor=PR39419_DFLASH_ANCHOR,
                    replacement=PR39419_DFLASH_REPLACEMENT,
                    required=True,
                ),
            ],
            upstream_drift_markers=[
    ...
    return "applied", (
        "PR39419 applied: get_top_tokens() added to qwen3.py + qwen3_dflash.py "
...
def is_applied() -> bool:
    ...
    try:
        with open(str(qwen3)) as f:
            return GENESIS_PR39419_MARKER in f.read()
    except OSError:
        return False
