...
GENESIS_PR40768_MARKER = "Genesis PR40768 async-scheduler -1 placeholder fix v7.59_p62_compat"
...
REQUEST_FIELD_NEW = (
    "        self.spec_token_ids: list[int] = []\n"
    "        # [Genesis PR40768] Backport vllm#40768: track async placeholder intent\n"
...
REQUEST_NUM_TOKENS_NEW = (
    "    def num_tokens_with_spec(self) -> int:\n"
    "        # [Genesis PR40768] Include async placeholder count for budget accounting.\n"
    "        return (\n"
...
def _make_request_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40768 request.py — placeholder counter field",
        target_file=str(target),
        marker=GENESIS_PR40768_MARKER + " :: request.py",
        sub_patches=[
            TextPatch(
...
ASYNC_SCHED_NEW = (
    "            request.num_output_placeholders += 1 + cur_num_spec_tokens\n"
    "            # [Genesis PR40768] Backport vllm#40768: track placeholder intent as\n"
...
def _make_async_sched_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40768 async_scheduler.py — counter-based placeholder intent",
        target_file=str(target),
        marker=GENESIS_PR40768_MARKER + " :: async_scheduler.py",
        sub_patches=[
            TextPatch(
...
SCHED_SPEC_BLOCK_NEW = (
    "            # [Genesis PR40768] Backport vllm#40768: speculative decode related.\n"
    "            if (\n"
    "                request.spec_token_ids\n"
...
SCHED_NEW_METHOD_NEW = (
    ...
    "    ):\n"
    "        \"\"\"[Genesis PR40768] Backport vllm#40768.\n"
    "\n"
...
#
# 2026-04-28: SPLIT INTO TWO NARROW ANCHORS to coexist with PR36138.
# Old single SCHED_DRAFT_OLD/NEW anchor included the lines PR36138 rewrites
# (`should_advance(request)` block → `validate_tokens_reasoning_aware`).
# After PR36138 applies first, the old monolith anchor stops matching and the
# entire PR40768 patch group skips with `required_anchor_missing`. The fix:
# split PR40768's two zero-resets into independent narrow anchors that survive
# whether PR36138 ran or not.
#
# Site A anchor — surrounds the `if is_prefill_chunk:` ... `continue` block.
#   Both pre-PR36138 and post-PR36138 layouts contain this block unchanged.
...
#   `update_draft_token_ids_in_output`. PR36138's sched_udti rewrites the lines
#   ABOVE the final assignment but keeps the assignment itself unchanged,
#   so this 3-line boundary anchor matches whether PR36138 ran or not.
#   Verified unique on both pre-PR36138 and post-PR36138 layouts 2026-04-28.
...
def _make_scheduler_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40768 scheduler.py — placeholder gating + new method",
        target_file=str(target),
        marker=GENESIS_PR40768_MARKER + " :: scheduler.py",
        sub_patches=[
            TextPatch(
            ...
            # 2026-04-28 split into two narrow anchors so PR40768 coexists with PR36138.
...
def apply() -> tuple[str, str]:
    """Apply PR40768 wiring (3 files, all-or-nothing). Never raises.
    ...
    decision, reason = should_apply("PR40768")
    log_decision("PR40768", decision, reason)
    if not decision:
        ...
        for sp in p.sub_patches:
            if sp.required and sp.anchor not in content:
                return (
                    ...
                    "PR40768 cannot apply safely without re-anchoring.",
                )
    ...
    if applied + idempotent == 3:
        return "applied", (
            f"PR40768 backport applied: {applied} files modified, {idempotent} "
