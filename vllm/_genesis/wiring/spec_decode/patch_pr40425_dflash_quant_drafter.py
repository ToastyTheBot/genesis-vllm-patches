"""Wiring for PR40425 — DFlash drafter quantization support (vllm PR #40425 backport).
...
Composition (verified no conflicts):
  - PR40898 (DFlash SWA) — different file (dflash.py)
  - PN23 (combine_hidden_states cast) — different method, same file
  - PR40727 (aux layer +1) — different file
  - PN40-A (fused per-layer K-norm) — different anchor: PR40425 modifies
...
GENESIS_PR40425_MARKER = (
    "Genesis PR40425 DFlash drafter quantization (PR #40425 backport)"
)
...
PR40425_A_REPLACEMENT = (
    "        # [Genesis PR40425 Site A] PR #40425: quant-aware QKV projection\n"
...
PR40425_B_REPLACEMENT = (
    "                    current_vllm_config,\n"
    "                    prefix=maybe_prefix(prefix, f\"layers.{layer_idx + start_layer_id}\"),\n"
    "                    config=self.config,\n"
    "                    quant_config=self.quant_config,  # [Genesis PR40425 Site B] PR #40425\n"
    "                )\n"
    "                for layer_idx in range(self.config.num_hidden_layers)\n"
)
...
PR40425_C_REPLACEMENT = (
    "        # [Genesis PR40425 Site C] PR #40425: conditional fused-KV buffers.\n"
...
PR40425_D_REPLACEMENT = (
    "            self._hidden_norm_weight,\n"
    "            self._rms_norm_eps,\n"
    "        )\n"
    "        # [Genesis PR40425 Site D] PR #40425: quantized DFlash drafter fallback\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40425 qwen3_dflash.py — quantized drafter support (PR #40425)",
        target_file=str(target),
        marker=GENESIS_PR40425_MARKER,
        sub_patches=[
            TextPatch(
                name="pN38_a_qkv_proj_call",
                anchor=PR40425_A_ANCHOR,
                replacement=PR40425_A_REPLACEMENT,
                required=True,
            ),
            TextPatch(
                name="pN38_b_pass_quant_config",
                anchor=PR40425_B_ANCHOR,
                replacement=PR40425_B_REPLACEMENT,
                required=True,
            ),
            TextPatch(
                name="pN38_c_conditional_fused_kv",
                anchor=PR40425_C_ANCHOR,
                replacement=PR40425_C_REPLACEMENT,
                required=True,
            ),
            TextPatch(
                name="pN38_d_quant_fallback",
                anchor=PR40425_D_ANCHOR,
                replacement=PR40425_D_REPLACEMENT,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40425 — quantized DFlash drafter support."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR40425")
    log_decision("PR40425", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR40425 applied: 4 sub-patches into qwen3_dflash.py — quantized "
