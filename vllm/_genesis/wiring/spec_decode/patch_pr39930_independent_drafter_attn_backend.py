...
`GENESIS_PR39930_DRAFTER_BACKEND`. Unset (default) → None → drafter
...
- Worst-case regression: if the auto-selected drafter backend turns
  ...
  recover: set `GENESIS_PR39930_DRAFTER_BACKEND=<target's backend name>`
...
GENESIS_PR39930_MARKER_PREFIX = (
    "Genesis PR39930 independent drafter attention backend (vllm#39930) v7.62.x"
)
...
def _read_drafter_backend() -> str | None:
    ...
    """
    raw = os.environ.get("GENESIS_PR39930_DRAFTER_BACKEND", "").strip()
...
def _marker_for(backend: str | None) -> str:
    ...
    GENESIS_PR39930_DRAFTER_BACKEND between restarts, idempotency check
    ...
    """
    return f"{GENESIS_PR39930_MARKER_PREFIX} backend={backend or 'auto'}"
...
def _build_replacement(backend: str | None) -> str:
    ...
    return (
        ...
        "        Subclasses may override to apply additional config changes.\n"
        "\n"
        "        [Genesis PR39930 vllm#39930 backport] Always reset the drafter's\n"
        ...
        "                    \"genesis.wiring.pN9\"\n"
        "                ).warning(\n"
        "                    \"[Genesis PR39930] could not resolve backend %r: %s; \"\n"
        ...
        "            )\n"
        "\n"
        "        # [Genesis PR39930] Reset drafter's attention backend.\n"
        "        # Baked from env GENESIS_PR39930_DRAFTER_BACKEND at server start.\n"
        ...
        "                \"genesis.wiring.pN9\"\n"
        "            ).warning(\n"
        "                \"[Genesis PR39930] attention_config replace failed: %s; \"\n"
...
def _make_patcher(backend: str | None) -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR39930 v1/spec_decode/llm_base_proposer.py — independent drafter "
...
def apply() -> tuple[str, str]:
    """Apply PR39930 — independent drafter attention backend (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR39930")
    log_decision("PR39930", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR39930] marker present (current backend) — skip (idempotent)")
        return "applied", "idempotent (marker present, backend unchanged)"

    # Stale-marker detection: a previous bake of PR39930 with a different
    ...
    if GENESIS_PR39930_MARKER_PREFIX in content:
        return (
            "skipped",
            f"PR39930 stale marker present (different backend). Container fs has a "
            f"previous PR39930 bake; current backend={backend or 'auto'} cannot be "
    ...
    return (
        "applied",
        f"PR39930 applied: drafter attention backend reset to "
...
def is_applied() -> bool:
    """Return True iff our PR39930 marker prefix is present in the target file."""
    if vllm_install_root() is None:
        return False
    target = resolve_vllm_file("v1/spec_decode/llm_base_proposer.py")
    if target is None:
    ...
    try:
        with open(target) as f:
            return GENESIS_PR39930_MARKER_PREFIX in f.read()
    except Exception:
        return False
