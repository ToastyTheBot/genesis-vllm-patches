...
- Unbiased: same target marginal as per-token rule (Sun 2024 §4 theorem).
  At T=0 (greedy) we skip PR40819 entirely (path returns earlier in upstream).
...
GENESIS_PR40819_MARKER = "Genesis PR40819 block-verify rejection sampling vllm#40819 v7.42_with_gemini_fixes"
...
PR40819_NEW = (
    ...
    "\n"
    "    # ════════════════════════════════════════════════════════════════\n"
    "    # [Genesis PR40819 vllm#40819] Block-verify rejection sampling (Sun 2024)\n"
    ...
    "                'GENESIS_PR40819_USE_PYTORCH', '0') == '1'\n"
    ...
    "            '[Genesis PR40819] block-verify failed (%s: %s); falling back to '\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40819 v1/sample/rejection_sampler.py — block-verify branch (vllm#40819)",
        target_file=str(target),
        marker=GENESIS_PR40819_MARKER,
        sub_patches=[
            TextPatch(
                name="p71_block_verify_branch",
                anchor=PR40819_OLD,
                replacement=PR40819_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40819 — block-verify rejection sampling."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR40819")
    log_decision("PR40819", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR40819] marker present — skip (idempotent)")
        return "applied", "idempotent (marker present)"
    for m in patcher.upstream_drift_markers:
        if m == "[Genesis PR40819" and m in content:
