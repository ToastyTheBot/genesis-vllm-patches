...
"""Wiring for PR40962 — vllm#40962 backport: spec-decode reasoning boundary validation.
...
PR36138 (vllm#36138 sfbemerk) — наш existing **broader** pipeline-level fix.
...
- **PR36138 (broader)**: more correct (per-position grammar masks), more
  invasive, slightly slower hot-path (more validation calls per step)
- **PR40962 (narrower)**: less correct in edge cases (only commit-time check,
...
enforces: PR40962 SKIPS if PR36138 active.

Genesis recommended:
- DEFAULT: PR36138 ON (broader safety, our PROD validated)
- ALTERNATIVE: PR36138 OFF + PR40962 ON (if you measure perf hit from PR36138
...
When upstream merges #36138 (PR36138 source) → PR36138 detects drift and SKIPS.
When upstream merges #40962 (PR40962 source) → PR40962 detects drift and SKIPS.
...
GENESIS_PR40962_MARKER = "Genesis PR40962 spec-decode reasoning boundary (vllm#40962)"
...
ENVS_NEW = (
    ...
    "        int(os.getenv(\"VLLM_LORA_ENABLE_DUAL_STREAM\", \"0\"))\n"
    "    ),\n"
    "    # [Genesis PR40962 vllm#40962] Reasoning-boundary validation for accepted\n"
...
ABS_PARSER_NEW = (
    "        return self.is_reasoning_end(input_ids)\n"
    "\n"
    "    # [Genesis PR40962 vllm#40962] reasoning-boundary detection in spec tokens\n"
...
BASIC_PARSER_NEW = (
    ...
    "        return end_token_id in delta_ids\n"
    "\n"
    "    # [Genesis PR40962 vllm#40962] efficient overrides for single-token markers\n"
...
STRUCT_OUT_NEW = (
    ...
    "\n"
    "\n"
    "# [Genesis PR40962 vllm#40962] reasoning-boundary validation for spec tokens\n"
...
SCHED_IMPORT_NEW = (
    ...
    "    # [Genesis PR40962 vllm#40962]\n"
    "    validate_spec_tokens_with_reasoning_boundary,\n"
    ")"
)

# Sub-E2: replace `if new_token_ids and ... should_advance(request):` block.
# This is the SAME block PR36138 replaces — mutual exclusion enforced by
# apply check above (PR36138 OFF required).
SCHED_VALIDATE_OLD = (
...
SCHED_VALIDATE_NEW = (
    ...
    "            # is grammar-validated BEFORE commit. Narrower than PR36138 broader fix.\n"
    ...
    "                # [Genesis PR40962 audit A-01 fix] use vllm.envs (already imported as `envs` in scheduler)\n"
...
def _make_envs_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40962 envs.py",
        target_file=str(target),
        marker=GENESIS_PR40962_MARKER + " (envs)",
...
def _make_abs_parser_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40962 abs_reasoning_parsers.py",
        target_file=str(target),
        marker=GENESIS_PR40962_MARKER + " (abs)",
...
def _make_basic_parser_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40962 basic_parsers.py",
        target_file=str(target),
        marker=GENESIS_PR40962_MARKER + " (basic)",
...
def _make_struct_out_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40962 structured_output/__init__.py",
        target_file=str(target),
        marker=GENESIS_PR40962_MARKER + " (struct_out)",
...
def _make_sched_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40962 scheduler.py (import + validate block)",
        target_file=str(target),
        marker=GENESIS_PR40962_MARKER + " (sched)",
        sub_patches=[
...
def apply() -> tuple[str, str]:
    ...
    decision, reason = should_apply("PR40962")
    log_decision("PR40962", decision, reason)
    if not decision:
        return "skipped", reason

    # Mutual exclusion gate — PR36138 patches same scheduler block.
    if _is_p62_active():
        return "skipped", (
    ...
    # partial state if PR36138 left scheduler.py modified.
    ...
    txn = MultiFilePatchTransaction(patchers, name="PR40962")
