...
GENESIS_PR39598_MARKER = "Genesis PR39598 qwen3coder MTP streaming early-return fix v7.13"
...
QWEN3CODER_NEW = (
    "            # [Genesis PR39598 vllm#39598] Do NOT early-return here. With MTP\n"
...
QWEN3COD_FNEND_NEW = (
    "                # [Genesis PR39598 vllm#39598] Append \"}\" to combined and fall\n"
...
SERVING_SHOULD_NEW = (
    "        # [Genesis PR39598 vllm#39598] Widen safety-net: with MTP/spec-decode\n"
...
# PR39598's widened _should_check returns True on finish_reason alone (no tool_calls
...
SERVING_CALLSITE_NEW = (
    "                        if should_check and tool_parser and auto_tools_called:\n"
    "                            latest_delta_len = 0\n"
    "                            # [Genesis PR39598 call-site guard] _should_check\n"
...
SERVING_CRD_NEW = (
    "        # [Genesis PR39598 vllm#39598] Use post-construction assignment\n"
    ...
    "        # [Genesis PR39598] above replaces the original constructor-style:\n"
    "        return DeltaMessage(\n"
    "            tool_calls=[\n"
    "                DeltaToolCall(\n"
    "                    index=index,\n"
...
def _make_qwen3cod_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR39598 qwen3coder_tool_parser.py — MTP streaming early-return removal",
        target_file=str(target),
        marker=GENESIS_PR39598_MARKER + " :: qwen3coder_tool_parser.py",
        sub_patches=[
...
def _make_serving_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR39598 serving.py — MTP safety-net + Pydantic null fix",
        target_file=str(target),
        marker=GENESIS_PR39598_MARKER + " :: serving.py",
        sub_patches=[
...
def apply() -> tuple[str, str]:
    """Apply PR39598 streaming-fix backport. All-or-nothing across two files."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR39598")
    log_decision("PR39598", decision, reason)
    if not decision:
        ...
        for sp in p.sub_patches:
            if sp.required and sp.anchor not in content:
                return (
                    ...
                    f"{p.target_file} — anchor drifted, PR39598 cannot apply.",
                )
    ...
    return "applied", (
        f"PR39598 applied: {applied} files modified, {idempotent} idempotent. "
