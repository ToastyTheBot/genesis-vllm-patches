...
output that PR40738+PR40738b+P61 didn't fully resolve.
================================================================
...
GENESIS_PR36138_MARKER = "Genesis PR36138 structured-output spec-decode timing v7.70 (post-PR41199 update — uses _get_reasoner)"
...
GRAMMAR_BITMASK_NEW = (
    "                # [Genesis PR36138 vllm#36138, v7.70 post-PR41199] reasoning-aware bitmask + advance\n"
    "                req_tokens = scheduled_spec_decode_tokens.get(req_id, ())\n"
...
NEW_METHODS_NEW = (
    "        return False\n"
    "\n"
    "    # ── [Genesis PR36138 vllm#36138, v7.70 post-PR41199] reasoning-aware spec-decode helpers ──\n"
    ...
    "    # (per-request lazy build with chat_template_kwargs). All PR36138 helpers updated\n"
...
def _make_struct_out_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR36138 structured_output/__init__.py",
        target_file=str(target),
        marker=GENESIS_PR36138_MARKER + " :: structured_output",
        sub_patches=[
...
SCHED_UPDATE_FROM_OUTPUT_NEW = (
    "            # [Genesis PR36138 vllm#36138] reasoning-aware grammar acceptance\n"
    ...
    "                        request.resumable = False\n"
    "                        stopped = True\n"
    "                # [Genesis PR36138 vllm#36138] update reasoning_ended after accept\n"
...
# (anchor matches BOTH original and post-PR40768 layout because PR40768 doesn't
...
SCHED_UDTI_NEW = (
    "            # [Genesis PR36138 vllm#36138] reasoning-aware spec validation\n"
    "            spec_token_ids = (\n"
...
def _make_scheduler_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR36138 scheduler.py",
        target_file=str(target),
        marker=GENESIS_PR36138_MARKER + " :: scheduler.py",
        sub_patches=[
...
def apply() -> tuple[str, str]:
    ...
    decision, reason = should_apply("PR36138")
    log_decision("PR36138", decision, reason)
    if not decision:
        ...
        for sp in p.sub_patches:
            if sp.required and sp.anchor not in content:
                return (
                    ...
                    f"{p.target_file} — anchor drifted, PR36138 cannot apply.",
                )
    ...
    return "applied", (
        f"PR36138 applied: {applied} files modified, {idempotent} idempotent. "
