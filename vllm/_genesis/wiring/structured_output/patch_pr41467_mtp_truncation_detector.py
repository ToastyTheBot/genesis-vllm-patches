...
"""Wiring for PR41467 — vllm#41467 backport: MTP truncation detector.
...
config which author reported. PR41467 — safety net на уже defended path
(PR39055/PR40738/P61/PR36138/PR39598/P68/P69 family).
...
GENESIS_PR41467_MARKER = "Genesis PR41467 MTP truncation detector (vllm#41467)"
...
ANCHOR_NEW = (
    ...
    "                            )\n"
    "\n"
    "                        # [Genesis PR41467 vllm#41467] MTP truncation detector.\n"
    ...
    "                        ):\n"
    "                            from vllm.entrypoints.openai.chat_completion.protocol import GenerationError as _PR41467_GenError\n"
    "                            logger.warning(\n"
    "                                \"[Genesis PR41467] MTP truncation detected for request %s: \"\n"
    "                                \"finished with 'stop' but tools configured and only \"\n"
    "                                \"reasoning produced.\",\n"
    "                                request_id,\n"
    "                            )\n"
    "                            raise _PR41467_GenError(\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR41467 MTP truncation detector (vllm#41467)",
        target_file=str(target),
        marker=GENESIS_PR41467_MARKER,
...
def apply() -> tuple[str, str]:
    ...
    decision, reason = should_apply("PR41467")
    log_decision("PR41467", decision, reason)
    if not decision:
    ...
    if result == TextPatchResult.APPLIED:
        return "applied", "PR41467 applied: MTP truncation now raises retryable error"
