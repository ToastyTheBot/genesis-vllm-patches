...
GENESIS_PR41446_MARKER = (
    "Genesis PR41446 GDN chunk_o scale-fold (vllm#41446 pattern c) v7.65"
)
...
PR41446_REPLACEMENT = (
    "    # [Genesis PR41446 vllm#41446 pattern (c) backport]\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR41446 model_executor/layers/fla/ops/chunk_o.py — "
            "scale-fold in chunk_fwd_kernel_o (vllm#41446 pattern c)"
        ),
        target_file=str(target),
        marker=GENESIS_PR41446_MARKER,
        sub_patches=[
            TextPatch(
                name="pN29_chunk_o_scale_fold",
                anchor=PR41446_ANCHOR,
                replacement=PR41446_REPLACEMENT,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR41446 — GDN chunk_o scale-fold (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR41446")
    log_decision("PR41446", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR41446 applied: chunk_fwd_kernel_o now uses scale-fold "
