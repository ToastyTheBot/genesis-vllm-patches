...
GENESIS_PR40385_MARKER = (
    "Genesis PR40385 Marlin TP cudagraph cap on Ampere (vllm#40385) v7.62.12"
)
...
PR40385_IMPORT_NEW = (
    ...
    "            and self.compilation_config.cudagraph_mode != CUDAGraphMode.NONE\n"
    "        ):\n"
    "            # [Genesis PR40385 vllm#40385 backport] need current_platform here\n"
...
PR40385_CAP_NEW = (
    ...
    "\n"
    "            # ════════════════════════════════════════════════════════════════\n"
    "            # [Genesis PR40385 vllm#40385 backport] Mitigation for illegal memory\n"
    ...
    "                        logger.warning_once(\n"
    "                            '[Genesis PR40385] Capping max_cudagraph_capture_size to 8 '\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40385 vllm/config/vllm.py — Marlin TP cudagraph cap (vllm#40385)",
        target_file=str(target),
        marker=GENESIS_PR40385_MARKER,
        sub_patches=[
            TextPatch(
                name="p95_import_current_platform",
                anchor=PR40385_IMPORT_OLD,
                replacement=PR40385_IMPORT_NEW,
                required=True,
            ),
            TextPatch(
                name="p95_marlin_cap_block",
                anchor=PR40385_CAP_OLD,
                replacement=PR40385_CAP_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40385 — Marlin TP cudagraph cap on Ampere."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR40385")
    log_decision("PR40385", decision, reason)
    if not decision:
    ...
    if patcher.marker in content:
        log.info("[PR40385] marker present — skip (idempotent)")
    ...
    return (
        "applied",
        "PR40385 v7.62.12 applied: vllm/config/vllm.py _set_cudagraph_sizes now "
