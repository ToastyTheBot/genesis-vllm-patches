...
PR40849 closes Cliff 1 on FP8 by freeing ~600 MiB persistent draft VRAM,
giving the heap enough slack for the 138 MiB transient. On TQ3 PR40849
...
- Worst case if PR34207 silently misbehaves: pool buffer not zeroed between
...
GENESIS_PR34207_MARKER = (
    "Genesis PR34207 FFN intermediate scratch pool (Cliff 1 fix) v7.62.x"
)
...
PR34207_SILU_REPLACEMENT = (
    "    def forward_cuda(self, x: torch.Tensor) -> torch.Tensor:\n"
    "        # [Genesis PR34207 FFN intermediate pool — Cliff 1 fix on TQ3]\n"
    ...
    "                FFNIntermediateCache as _GENESIS_PR34207_Cache,\n"
    "            )\n"
    "            if _GENESIS_PR34207_Cache.is_production_eligible() and x.dim() == 2:\n"
    "                out = _GENESIS_PR34207_Cache.acquire_silu_out(\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR34207 model_executor/layers/activation.py — SiluAndMul "
            "forward_cuda FFN intermediate pool (Cliff 1 fix on TQ3)"
        ),
        target_file=str(target),
        marker=GENESIS_PR34207_MARKER,
        sub_patches=[
            TextPatch(
                name="pN12_silu_and_mul_pool",
                anchor=PR34207_SILU_ANCHOR,
                replacement=PR34207_SILU_REPLACEMENT,
                required=True,
            ),
        ],
        upstream_drift_markers=[
            ...
            # with PR34207 on the same file. Idempotency is handled by the
            # Genesis-PR34207 wiring marker line above.
        ],
    )


def apply() -> tuple[str, str]:
    """Apply PR34207 — FFN intermediate scratch pool (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR34207")
    log_decision("PR34207", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR34207 applied: SiluAndMul.forward_cuda now acquires output via "
            "FFNIntermediateCache pool when GENESIS_ENABLE_PR34207_FFN_"
            "INTERMEDIATE_POOL=1. Closes Cliff 1 OOM on TQ3 path that PR40849 "
