...
GENESIS_PR41142_MARKER = (
    "Genesis PR41142 GDN a/b contiguity (vllm#41142) v7.62.x"
)
...
PR41142_REPLACEMENT = (
    "        # [Genesis PR41142 vllm#41142 backport] Force contiguity:\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR41142 model_executor/layers/mamba/gdn_linear_attn.py — "
            "force a/b contiguity in fix_query_key_value_ordering (vllm#41142)"
        ),
        target_file=str(target),
        marker=GENESIS_PR41142_MARKER,
        sub_patches=[
            TextPatch(
                name="pN11_a_b_contiguous",
                anchor=PR41142_ANCHOR,
                replacement=PR41142_REPLACEMENT,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR41142 — force GDN a/b contiguity (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR41142")
    log_decision("PR41142", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR41142 applied: GatedDeltaNetAttention.fix_query_key_value_ordering "
