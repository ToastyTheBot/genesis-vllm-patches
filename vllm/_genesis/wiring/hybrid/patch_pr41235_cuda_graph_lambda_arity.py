...
GENESIS_PR41235_MARKER = (
    "Genesis PR41235 CUDAGraphWrapper lambda arity (vllm#41235) v7.62.x"
)
...
PR41235_REPLACEMENT = (
    "                    # [Genesis PR41235 vllm#41235 backport] gc.collect is\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR41235 compilation/cuda_graph.py — CUDAGraphWrapper gc.collect/"
            "empty_cache lambda arity fix (vllm#41235)"
        ),
        target_file=str(target),
        marker=GENESIS_PR41235_MARKER,
        sub_patches=[
            TextPatch(
                name="pN13_lambda_arity",
                anchor=PR41235_ANCHOR,
                replacement=PR41235_REPLACEMENT,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR41235 — CUDAGraphWrapper lambda arity fix (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR41235")
    log_decision("PR41235", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR41235 applied: CUDAGraphWrapper.__call__ now accepts var-args on "
