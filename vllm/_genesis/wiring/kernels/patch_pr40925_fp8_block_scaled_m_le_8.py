...
GENESIS_PR40925_MARKER = "Genesis PR40925 fp8 block-scaled M<=8 backport vllm#40925 v7.47"
...
PR40925_NEW = (
    ...
    "        # BLOCK_SIZE_K must be divisible by block_size[1]\n"
    "        # ════════════════════════════════════════════════════════════\n"
    "        # [Genesis PR40925 backport vllm#40925] M-aware tuning for low-M decode.\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    if target is None:
        return None
    return TextPatcher(
        patch_name="PR40925 fp8_utils.py — fp8 block-scaled M<=8 tuning backport",
        target_file=str(target),
        marker=GENESIS_PR40925_MARKER,
        sub_patches=[
            TextPatch(
                name="p81_fp8_block_scaled_m_le_8",
                anchor=PR40925_OLD,
                replacement=PR40925_NEW,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40925 — fp8 block-scaled M<=8 tuning."""
    from vllm._genesis.dispatcher import should_apply, log_decision
    decision, reason = should_apply("PR40925")
    log_decision("PR40925", decision, reason)
    if not decision:
    ...
    return "applied", (
        "PR40925 applied: w8a8_triton_block_scaled_mm default config now uses "
