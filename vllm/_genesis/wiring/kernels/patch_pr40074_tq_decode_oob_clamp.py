...
GENESIS_PR40074_MARKER = (
    "Genesis PR40074 TQ decode safe_page_idx clamp (vllm#40074) v7.62.x"
)
...
PR40074_REPLACEMENT = (
    "        page_idx = kv_offs // BLOCK_SIZE\n"
    "        page_off = kv_offs % BLOCK_SIZE\n"
    "        # [Genesis PR40074 vllm#40074 backport] Clamp OOB lanes to page_idx=0\n"
...
def _make_patcher() -> TextPatcher | None:
    ...
    return TextPatcher(
        patch_name=(
            "PR40074 triton_turboquant_decode.py — TQ decode IOOB safe_page_idx "
            "clamp (vllm#40074)"
        ),
        target_file=str(target),
        marker=GENESIS_PR40074_MARKER,
        sub_patches=[
            TextPatch(
                name="pN14_safe_page_idx",
                anchor=PR40074_ANCHOR,
                replacement=PR40074_REPLACEMENT,
                required=True,
            ),
        ],
...
def apply() -> tuple[str, str]:
    """Apply PR40074 — TQ decode IOOB safe_page_idx clamp (text-patch)."""
    from vllm._genesis.dispatcher import log_decision, should_apply

    decision, reason = should_apply("PR40074")
    log_decision("PR40074", decision, reason)
    if not decision:
    ...
    return result_to_wiring_status(
        result, failure,
        applied_message=(
            "PR40074 applied: _tq_decode_stage1 now clamps masked-out lanes to "
