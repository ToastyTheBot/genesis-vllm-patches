...
"""TDD tests for PR41446 — GDN chunk_o scale-fold (vllm#41446 pattern (c) backport).
...
def test_pn29_wiring_imports():
    """PR41446 wiring module imports cleanly."""
    from vllm._genesis.wiring.hybrid import patch_pr41446_gdn_chunk_o_scale_fold
    assert hasattr(patch_pr41446_gdn_chunk_o_scale_fold, "apply")
    assert hasattr(patch_pr41446_gdn_chunk_o_scale_fold, "GENESIS_PR41446_MARKER")


def test_pn29_dispatcher_registry():
    """PR41446 registered in PATCH_REGISTRY with correct env flag."""
...
def test_pn29_anchor_text_matches_upstream():
    """PR41446 anchor matches exact upstream chunk_o.py:137 line."""
    from vllm._genesis.wiring.hybrid.patch_pr41446_gdn_chunk_o_scale_fold import (
        PR41446_ANCHOR, PR41446_REPLACEMENT,
    )
    # Anchor: the EXACT current upstream line
    assert "b_o = b_o * scale + tl.dot(b_A.to(b_v.dtype), b_v) * scale" in PR41446_ANCHOR
    # Replacement: scale-fold form
    assert "b_o = (b_o + tl.dot(b_A.to(b_v.dtype), b_v)) * scale" in PR41446_REPLACEMENT
    # Drift marker
    assert "Genesis PR41446" in PR41446_REPLACEMENT
    assert "vllm#41446" in PR41446_REPLACEMENT


def test_pn29_marker_string_unique():
    """PR41446 marker is non-trivial for drift detection."""
    from vllm._genesis.wiring.hybrid.patch_pr41446_gdn_chunk_o_scale_fold import (
        GENESIS_PR41446_MARKER,
    )
    assert "PR41446" in GENESIS_PR41446_MARKER
    assert len(GENESIS_PR41446_MARKER) > 30


def test_pn29_register_in_apply_all():
    """PR41446 registered via @register_patch in apply_all.py."""
    from vllm._genesis.patches.apply_all import PATCH_REGISTRY as APPLY_REGISTRY
    names = [name for name, _ in APPLY_REGISTRY]
    pn29 = [n for n in names if "PR41446" in n]
    assert len(pn29) == 1, f"PR41446 not registered, names: {names[:5]}"
...
def test_pn29_idempotency_via_marker():
    """Re-applying PR41446 doesn't double-patch (marker check)."""
    from vllm._genesis.wiring.hybrid.patch_pr41446_gdn_chunk_o_scale_fold import (
        GENESIS_PR41446_MARKER,
    )
    ...
    assert GENESIS_PR41446_MARKER.startswith("Genesis PR41446")
