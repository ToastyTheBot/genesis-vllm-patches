...
"""TDD for PR41411 — vllm#41411 backport (multi-file).
...
def _wiring():
    from vllm._genesis.wiring.perf_hotfix import (
        patch_pr41411_prompt_logprobs_eviction as M,
    )
    return M
...
def test_replacements_carry_pn52_marker():
    ...
    for n, new in [
        ...
        assert "PR41411" in new, f"{n} missing PR41411 marker"
...
def test_idempotent_on_synthetic_each_file(tmp_path):
    ...
    for fname, old, new, sub_name in cases:
        ...
        patcher = TextPatcher(
            ...
            marker=M.GENESIS_PR41411_MARKER,
        ...
        assert "PR41411" in body1, f"{fname} marker missing"
        r2, _ = patcher.apply()
...
def test_apply_all_registers_pn52():
    ...
    from vllm._genesis.dispatcher import PATCH_REGISTRY
    entry = PATCH_REGISTRY["PR41411"]
    assert entry["wiring"] == "patch_pr41411_prompt_logprobs_eviction" and callable(entry.get("apply_callable"))