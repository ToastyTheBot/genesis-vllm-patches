...
"""TDD tests for PR41418 Genesis sparse-V kernel.
...
requires_cuda = pytest.mark.skipif(
    ...
    reason="GPU + vllm.triton_utils required for PR41418 sparse-V kernel tests",
)
...
def test_default_threshold_value(monkeypatch):
    ...
    monkeypatch.delenv("GENESIS_PR41418_SPARSE_V_THRESHOLD", raising=False)
    monkeypatch.delenv("GENESIS_PR41418_SPARSE_V_SCALE_FACTOR", raising=False)
...
def test_threshold_clamping_invalid(monkeypatch):
    ...
    monkeypatch.setenv("GENESIS_PR41418_SPARSE_V_THRESHOLD", "999.0")
    from vllm._genesis.kernels.triton_turboquant_decode_sparse_v import (
        get_sparse_v_threshold,
    )
    assert get_sparse_v_threshold() == 0.001
...
def test_blasst_lambda_scaling(monkeypatch):
    ...
    monkeypatch.setenv("GENESIS_PR41418_SPARSE_V_SCALE_FACTOR", "10.0")
    from vllm._genesis.kernels.triton_turboquant_decode_sparse_v import (
...
def test_min_ctx_default(monkeypatch):
    ...
    monkeypatch.delenv("GENESIS_PR41418_SPARSE_V_MIN_CTX", raising=False)
    from vllm._genesis.kernels.triton_turboquant_decode_sparse_v import (
...
def test_wiring_patch_imports():
    ...
    assert hasattr(patch_pr41418_sparse_v_kernel, "apply")
    assert hasattr(patch_pr41418_sparse_v_kernel, "is_applied")
...
def test_dispatcher_registry_entry():
    """PR41422 is registered in dispatcher PATCH_REGISTRY."""
