...
"""TDD tests for PR35975 — skip inputs_embeds buffer for text-only models.

Verifies:
  - Wiring module imports cleanly + exposes apply()
  - Dispatcher registry + apply_all integration
  - Default-on (env-flag GENESIS_DISABLE_PR35975_INPUTS_EMBEDS_OPTIONAL
...
def test_pn35_wiring_module_imports():
    from vllm._genesis.wiring.perf_hotfix import (
        patch_pr35975_inputs_embeds_optional as mod,
    )
    assert hasattr(mod, "apply")
    assert hasattr(mod, "GENESIS_PR35975_MARKER")
    assert hasattr(mod, "PR35975_PART1_ANCHOR")
    assert hasattr(mod, "PR35975_PART2_ANCHOR")
...
def test_pn35_registered_in_apply_all():
    ...
    found = [n for n in names if "PR35975" in n]
    assert len(found) == 1, (
        f"PR35975 not registered in apply_all (or duplicated). "
        f"Names matching 'PR35975': {found}"
    )
...
def test_pn35_credit_mentions_upstream_author_and_noonghunna():
    ...
    credit = PATCH_REGISTRY["PR35975"]["credit"]
...
def test_pn35_part1_anchor_targets_gpu_model_runner_make_buffer():
    ...
    from vllm._genesis.wiring.perf_hotfix.patch_pr35975_inputs_embeds_optional import (
        PR35975_PART1_ANCHOR,
    )
    assert "self.inputs_embeds = self._make_buffer(" in PR35975_PART1_ANCHOR
    assert "self.max_num_tokens" in PR35975_PART1_ANCHOR
    assert "self.inputs_embeds_size" in PR35975_PART1_ANCHOR
    assert "numpy=False" in PR35975_PART1_ANCHOR


def test_pn35_part2_anchor_targets_llm_base_proposer_torch_zeros():
    """part2 must match torch.zeros(...) in llm_base_proposer.py:205."""
    from vllm._genesis.wiring.perf_hotfix.patch_pr35975_inputs_embeds_optional import (
        PR35975_PART2_ANCHOR,
    )
    assert "self.inputs_embeds = torch.zeros(" in PR35975_PART2_ANCHOR
    assert "self.max_num_tokens, self.inputs_embeds_size" in PR35975_PART2_ANCHOR
    assert "device=device," in PR35975_PART2_ANCHOR
...
def test_pn35_part1_replacement_preserves_original_under_guard():
    ...
    from vllm._genesis.wiring.perf_hotfix.patch_pr35975_inputs_embeds_optional import (
        PR35975_PART1_REPLACEMENT,
    )
    # Default value: None
    assert "self.inputs_embeds = None" in PR35975_PART1_REPLACEMENT
    # Guard re-allocates for multimodal OR prompt_embeds
    assert "self.supports_mm_inputs or self.enable_prompt_embeds" in PR35975_PART1_REPLACEMENT
    # Original allocation preserved inside the guard
    assert "self._make_buffer(" in PR35975_PART1_REPLACEMENT


def test_pn35_part2_replacement_preserves_original_under_guard():
    from vllm._genesis.wiring.perf_hotfix.patch_pr35975_inputs_embeds_optional import (
        PR35975_PART2_REPLACEMENT,
    )
    assert "self.inputs_embeds = None" in PR35975_PART2_REPLACEMENT
    assert "self.supports_mm_inputs" in PR35975_PART2_REPLACEMENT
    # Original torch.zeros call preserved under guard
    assert "torch.zeros(" in PR35975_PART2_REPLACEMENT
...
def test_pn35_marker_unique_and_versioned():
    from vllm._genesis.wiring.perf_hotfix.patch_pr35975_inputs_embeds_optional import (
        GENESIS_PR35975_MARKER,
    )
    assert "PR35975" in GENESIS_PR35975_MARKER
    assert "v7.69" in GENESIS_PR35975_MARKER
    assert "vllm#35975" in GENESIS_PR35975_MARKER


def test_pn35_drift_markers_specific_no_generic_collision_risk():
    """Drift markers must be SPECIFIC. Generic '[Genesis PR35975' would
    ...
    for patcher in [p1, p2]:
        ...
        for m in patcher.upstream_drift_markers:
            # Must NOT be a bare '[Genesis PR35975' prefix that could
...
def test_pn35_module_docstring_mentions_composition_with_cliff_2_stack():
    """PR35975 is particularly useful WITH P103 + PN32 (Cliff 2 stack).
...
def test_pn35_in_patches_md():
    """PATCHES.md must list PR35975 — operators browse PATCHES.md to
    ...
    assert "PR35975" in patches_md, (
        "PATCHES.md missing PR35975 — add a row in the perf_hotfix table"
    )
