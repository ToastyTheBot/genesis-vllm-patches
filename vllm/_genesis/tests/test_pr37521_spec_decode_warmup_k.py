...
"""TDD tests for PR37521 — spec-decode warmup K-aware sizing.
...
def test_pn33_wiring_imports():
    from vllm._genesis.wiring.spec_decode import patch_pr37521_spec_decode_warmup_k as mod
    assert hasattr(mod, "apply")
    assert hasattr(mod, "GENESIS_PR37521_MARKER")
    assert hasattr(mod, "PR37521_ANCHOR")
    assert hasattr(mod, "PR37521_REPLACEMENT")
...
def test_pn33_anchor_matches_canonical_upstream():
    from vllm._genesis.wiring.spec_decode.patch_pr37521_spec_decode_warmup_k import (
        PR37521_ANCHOR,
    )
    # Anchor is the original 1-draft-token warmup line in
    # gpu_model_runner._dummy_sampler_run(). Canonical formatting.
    assert "if self.speculative_config:" in PR37521_ANCHOR
    assert "draft_token_ids = [[0] for _ in range(num_reqs)]" in PR37521_ANCHOR
...
def test_pn33_replacement_uses_num_speculative_tokens():
    ...
    assert "num_speculative_tokens" in PR37521_REPLACEMENT
    assert "getattr(" in PR37521_REPLACEMENT
    # Distinct token IDs (matches upstream PR's choice — avoids dedup
    # under-counting in some sampler paths)
    assert "list(range(_genesis_pn33_K))" in PR37521_REPLACEMENT
...
def test_pn33_replacement_extends_beyond_eagle_only():
    ...
    assert "use_eagle()" not in PR37521_REPLACEMENT, (
        "PR37521 must NOT gate on use_eagle() — upstream's EAGLE-only fix "
...
def test_pn33_replacement_falls_through_when_K_is_1():
    ...
    assert "_genesis_pn33_dummy_tokens = [0]" in PR37521_REPLACEMENT
    # Guard: K > 1 (not >= 1; K=1 is degenerate same as [0])
    assert "_genesis_pn33_K > 1" in PR37521_REPLACEMENT


def test_pn33_replacement_honors_disable_env():
    """Operator can disable PR37521 via env if K-sized warmup OOMs on a
    tight rig. Disable env reverts to original [0]."""
    from vllm._genesis.wiring.spec_decode.patch_pr37521_spec_decode_warmup_k import (
        PR37521_REPLACEMENT,
    )
    assert "GENESIS_DISABLE_PR37521_SPEC_DECODE_WARMUP_K" in PR37521_REPLACEMENT
    # Honored values include common truthy strings
    assert "'1'" in PR37521_REPLACEMENT
    assert "'true'" in PR37521_REPLACEMENT
...
def test_pn33_register_in_apply_all():
    ...
    # Match PR37521 as standalone token (not substring) — PN34's title
    # references PR37521 ("companion to PR37521"), so plain `"PR37521" in n`
    # would also match PN34's entry.
    pn33 = [n for n in names if n.startswith("PR37521 ")]
    assert len(pn33) == 1, (
        f"PR37521 not registered in apply_all, "
...
def test_pn33_marker_unique():
    from vllm._genesis.wiring.spec_decode.patch_pr37521_spec_decode_warmup_k import (
        GENESIS_PR37521_MARKER,
    )
    assert "PR37521" in GENESIS_PR37521_MARKER
    # Version pin so re-applies after kernel rewrites don't no-op
    # against a stale marker
    assert "v7.65" in GENESIS_PR37521_MARKER


def test_pn33_documents_root_cause_coupling():
    """Source must document that PR37521 closes BOTH ampersandru's
