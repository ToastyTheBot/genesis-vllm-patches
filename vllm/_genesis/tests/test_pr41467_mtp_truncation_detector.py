...
"""TDD for PR41467 — MTP truncation detector (vllm#41467)."""
...
def _wiring():
    from vllm._genesis.wiring.structured_output import (
        patch_pr41467_mtp_truncation_detector as M,
    )
    return M
...
def test_replacement_adds_p107_guard():
    M = _wiring()
    assert "PR41467" in M.ANCHOR_NEW
...
def test_idempotent_on_synthetic(tmp_path):
    ...
    patcher = TextPatcher(
        patch_name="PR41467 test",
        target_file=str(target),
        marker=M.GENESIS_PR41467_MARKER,
...
def test_registry_entry_complete():
    ...
    assert "PR41467" in PATCH_REGISTRY
    meta = PATCH_REGISTRY["PR41467"]
