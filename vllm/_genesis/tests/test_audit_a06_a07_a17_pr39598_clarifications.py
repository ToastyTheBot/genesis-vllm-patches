...
"""Audit A-06 / A-07 / A-17 — PR39598 documentation + intentional-deferral test.

Background (audit 2026-05-05):
  A-06: PR39598 docstring обещает "3 sub-patches" but Pydantic/null fix
        (`_create_remaining_args_delta` → SERVING_CRD_NEW) is NOT in
        sub_patches list.
  A-07: PR39598 QWEN3COD_FNEND_NEW removes `self.json_closed = True` line
...
Findings after deep analysis:
  ...
  A-07: NOT a runtime bug — `self.json_closed = True` is set upstream
        ...
        BEFORE our PR39598 anchor's branch executes. Fix → add explanatory
        comment in PR39598 replacement.
...
def _wiring():
    from vllm._genesis.wiring.structured_output import (
        patch_pr39598_qwen3coder_mtp_streaming as M,
    )
    return M
...
def _wiring_source() -> str:
    from vllm._genesis.wiring.structured_output import (
        patch_pr39598_qwen3coder_mtp_streaming,
    )
    return open(patch_pr39598_qwen3coder_mtp_streaming.__file__).read()
