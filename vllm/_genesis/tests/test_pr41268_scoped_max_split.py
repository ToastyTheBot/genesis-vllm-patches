...
"""Tests for PR41268 — Scoped max_split_size_mb during model load.

PR41268 backports vllm#41268 (MatthewBonanni). PyTorch 2.10+ introduced
load-time allocator fragmentation; PR41268 mitigates by setting
...
- PR41268 is registered in PATCH_REGISTRY + apply_all + PATCHES.md
"""
...
class TestPN19ModuleStructure:
    def test_module_importable(self):
        from vllm._genesis.wiring.perf_hotfix import patch_pr41268_scoped_max_split  # noqa: F401
    ...
    def test_helper_anchor_targets_init_device(self):
        ...
        assert "@instrument(span_name=\"Init device\")" in PR41268_HELPER_OLD
        assert "def init_device(self):" in PR41268_HELPER_OLD
        # Replacement must KEEP these two lines (they survive after
        # injecting our helper above them)
        assert "@instrument(span_name=\"Init device\")" in PR41268_HELPER_NEW
        assert "def init_device(self):" in PR41268_HELPER_NEW
...
class TestPN19MaxSplitValue:
    def test_uses_pytorch_minimum_20mb(self):
        ...
        assert "max_split_size_mb=20" in LOAD_MODEL_NEW, (
            "PR41268 must use 20 MiB (PyTorch minimum) like upstream PR #41268. "
...
class TestPN19TorchVersionFallback:
    """torch < 2.11 lacks `_accelerator_setAllocatorSettings`. PR41268
    ...
    def test_helper_includes_torch_fallback(self):
        ...
        assert "torch._C._accelerator_setAllocatorSettings" in PR41268_HELPER_NEW
        # The setter call must be wrapped in try/except so older torch
        # falls through cleanly
        assert "except Exception" in PR41268_HELPER_NEW
        assert "yield" in PR41268_HELPER_NEW
    ...
    def test_helper_includes_cuda_check(self):
        ...
        assert "current_platform.is_cuda()" in PR41268_HELPER_NEW
    ...
    def test_helper_restores_prior_value(self):
        ...
        assert "original_value" in PR41268_HELPER_NEW
        assert "_SIZE_MAX_MB" in PR41268_HELPER_NEW or "SIZE_MAX" in PR41268_HELPER_NEW.upper()
        assert "finally:" in PR41268_HELPER_NEW
...
class TestPN19DispatcherIntegration:
    ...
    def test_pn19_category_is_memory_savings(self):
        from vllm._genesis.dispatcher import PATCH_REGISTRY
        assert PATCH_REGISTRY["PR41268"]["category"] == "memory_savings"
