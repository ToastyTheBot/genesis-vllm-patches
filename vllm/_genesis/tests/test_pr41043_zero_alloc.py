...
"""TDD test for PR41043 — backport of vllm#41043
...
from vllm._genesis.wiring.spec_decode.patch_pr41043_spec_decode_zero_alloc import (
    PR41043_OLD,
    PR41043_NEW,
    GENESIS_PR41043_MARKER,
)
...
def test_p94_old_has_anti_pattern_tolist():
    ...
    assert pat.search(PR41043_OLD), (
        "PR41043_OLD anchor missing .tolist() pattern — anchor obsolete"
    )
...
def test_p94_new_has_no_tolist_in_executable_code():
    ...
    code_lines = [
        line for line in PR41043_NEW.split("\\n")
        if not line.lstrip().startswith("#") and not line.startswith('    "        #')
    ]
    code_only = "\\n".join(code_lines)
    assert ".tolist()" not in code_only, (
        f"PR41043_NEW still contains .tolist() in executable code:\n{code_only}"
    )
...
def test_p94_new_has_no_np_array_alloc():
    ...
    assert "np.array(" not in code_only, (
        f"PR41043_NEW still contains np.array(...) allocation in executable "
        f"code:\n{code_only}"
    )
...
def test_p94_new_has_in_place_loop():
    ...
    assert pat_for.search(PR41043_NEW), "PR41043_NEW missing `for i in range(num_reqs)`"
    assert pat_assign.search(PR41043_NEW), (
        "PR41043_NEW missing in-place assignment `self.backup_next_token_ids.np[i] = requests[...]`"
    )
...
def test_p94_marker_versioned():
    ...
    bump the patch."""
    assert "v7.62" in GENESIS_PR41043_MARKER, (
        f"PR41043 marker {GENESIS_PR41043_MARKER!r} should embed v7.62.x"
    )
    assert "vllm#41043" in GENESIS_PR41043_MARKER, (
        "PR41043 marker should reference upstream PR for drift detection"
    )
